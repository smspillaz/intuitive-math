...
module.exports = (app, options) => {
  ...
  if (isProd) {
    import('./addProdMiddlewares').then(addProdMiddlewares =>
      addProdMiddlewares.default(app, options),
    );
  } else {
    import('./addDevMiddlewares').then(addDevMiddlewares => {
      import('../../internals/webpack/webpack.dev.client.babel').then(
        webpackConfig =>
          addDevMiddlewares.default(app, webpackConfig.default, options), // eslint-disable prettier/prettier
      );
    });
  }
