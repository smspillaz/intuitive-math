...
export const matmul = (a, b) => {
  if (a.length !== b[0].length) {
    throw new Error(
      `Cannot multiply matrices of left columns ${a.length} and right rows ${b[0].length}`,
    );
  }
...
export const interpolateArrays = (initial, destination, value) =>
  range(initial.length).map(
    i => initial[i] * (1 - value) + destination[i] * value,
  );
...
export const interpolateHomogenousMatrixFunc = (initial, destination) => {
  ...
  const [destinationPosition, destinationRotation, destinationScale] = [
    new Vector3(),
    new Quaternion(),
    new Vector3(),
  ];
  ...
  const [lerpPosition, lerpRotation, lerpScale] = [
    new Vector3(),
    new Quaternion(),
    new Vector3(),
  ];
