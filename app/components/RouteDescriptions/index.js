...
class RouteDescriptions extends React.Component {
  ...
  updateIndicesFromRoutePathname(pathname) {
    let routeIndex = -1;
    const categoryIndex = this.props.descriptions.categories.findIndex(
      category => {
        routeIndex = category.routes.findIndex(route =>
...
RouteDescriptions.childContextTypes = {
...
export const withRouteDescriptions = Component => {
  const Wrapped = (props, { routeDescriptions }) => (
    <Component routeDescriptions={routeDescriptions}></Component>
  );
