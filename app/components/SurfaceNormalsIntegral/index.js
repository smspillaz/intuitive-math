  ...
  <group>
    {segment1D(uMin, uMax, segments).map(([uBegin, uEnd]) => {
      ...
      return segment1D(vMin, vMax, segments).map(([vBegin, vEnd]) => {
        ...
        const uGradientVec = uGradientVecFunc(uCenter, vCenter).multiplyScalar(
          (uEnd - uBegin) * visualizationBias,
        );
        const vGradientVec = vGradientVecFunc(uCenter, vCenter).multiplyScalar(
          (vEnd - vBegin) * visualizationBias,
        );
        const normalVec = cross(
          [uGradientVec.x, uGradientVec.y, uGradientVec.z],
