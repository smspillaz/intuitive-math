    ...
    render={() => (
      <group>
        {segment1D(xMin, xMax, segments).map(([begin, end]) => {
          ...
          const height = Math.max(Math.min(yMax + Math.min(0, value), value) - yMin, 0);
...
export const SpreadingSingleIntegralVisualization = ({ startSegments, endSegments, ...props }) => (
...
export const RollingSingleIntegralVisualization = ({ xMin, xMax, ...props }) => (
  <InterpolatedAnimation
    values={{
    ...
    render={({ complete }) => (
      <SingleIntegralVisualization
        func={(x) => (-(1 / 2) * (x ** 2)) + 3}
        opacityFunc={(x) => ((x - xMin) / (xMax - xMin)) < complete.value ? 0.8 : 0.0}
        xMin={xMin}
        xMax={xMax}
        {...props}
      />
