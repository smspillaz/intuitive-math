...
const SpansSection = () => (
  <Section title="Spans" anchor="spans">
    ...
    <p>
      ...
      <Strong>linear combinations</Strong> of some given vectors. In fact, it is
      the set of all vectors reachable by linear combinations of vectors in the
      span.
    </p>
    <p>
      What makes this slightly annoying to think about is that that a span
      describes an infinite set and infinite sets are a little hard to reason
      about.
    </p>
    <p>
      But if we think about it geometrically, it is actually a case that we have
      seen before. Say for instance we have the following vectors in 2D space:
    </p>
    <AxisVisualization2D
    ...
    <p>
      The span of these two vectors is all of 2D space. The reason for that is
      that you can give me any 2D point and I can tell you two scalar multiples
      of these two vectors that will give you that point.
    </p>
    <p>
      For example, say you wanted the point{' '}
      <MathJax.Node inline formula="(4, 1)" />; I could say, well,{' '}
      <MathJax.Node inline formula="(4, 1)" /> is really just{' '}
      <MathJax.Node inline formula="4 \times (1, 0) + 1 \times (0, 1)" />
    </p>
        ...
        <AxisVisualization2D
          ...
          render={() => (
            <group>
              ...
              />
              <Vector
                position={new Vector3(zxInterp.value, zyInterp.value, 0)}
                color={0x00ffff}
              />
            </group>
          )}
    ...
    />
    <p>
      Now, what if you had the vectors{' '}
      <MathJaxMatrix inline matrix={[[1], [-1]]} /> and{' '}
      <MathJaxMatrix inline matrix={[[-1], [-1]]} />. These two vectors are not
      linearly dependent. Can we reach any point in 2D space using just
      combinations of those two? What about{' '}
      <MathJax.Node inline formula="(4, 1)" />?
    </p>
    <InterpolatedAnimation
      values={{
        ...
        <div>
          <AxisVisualization2D
            ...
            render={() => (
              <group>
                <Vector
                  position={new Vector3(xxInterp.value, xyInterp.value, 0)}
                  color={0xffff00}
                />
                <Vector
                  position={new Vector3(yxInterp.value, yyInterp.value, 0)}
                  color={0xff00ff}
                />
                <Vector
                  position={
                    new Vector3(
                      xxInterp.value + xyInterp.value,
    ...
    <p>
      ...
      <MathJax.Node inline formula="1.5" />
      <MathJaxMatrix inline matrix={[[1], [-1]]} /> and{' '}
      <MathJax.Node inline formula="-2.5" />
      <MathJaxMatrix inline matrix={[[-1], [-1]]} /> ; adding both gives me{' '}
      <MathJaxMatrix inline matrix={[[4], [4]]} />
    </p>
    <p>
    ...
    <p>
      ...
      other vector outside of that line), the we can only say that the span of
      those two vectors is the line defined by{' '}
      <MathJax.Node inline formula="{'(x, y): -x = y'}." />
    </p>
    <p>
      Of course, you can generalize to higher dimensions, for instance, the span
      of three vectors may be all of 3D space, or it might only be a plane if
      there was a linear dependence amongst the vectors.
    </p>
  </Section>
);
